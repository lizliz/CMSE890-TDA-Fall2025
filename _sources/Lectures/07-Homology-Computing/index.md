# Lecture 7

This lecture covers the basic ideas of computing homology from Smith Normal Form of the boundary matrix.

- **Reading:** EH IV.2
- **Slides:** 
  - No slides for this lecture. 
- **Code (Download both for class):**
  - [Lec07-ComputingHomology.ipynb](./Lec07-ComputingHomology.ipynb)
  - [smith.py](smith.py), a Python script for computing Smith Normal Form with $\mathbb{Z}_2$ coefficients.
